...
with Image.open(f'datasets/images/{split_name}/{image_id}.png') as f:
    ...
    for idx, (bbox, label) in enumerate(zip(bboxs,labels)):
        ...
        ax.add_patch(patches.Rectangle((bbox[0], bbox[1]),bbox[2],bbox[3], fill=False, edgecolor='blue', lw=3))
        ax.text(bbox[0]+10,(bbox[1]+10),str(label),verticalalignment='top',color='white',fontsize=10,weight='bold')
